...
// GenerateCompleteCSharp generates a complete C# file with all types, namespace, and usings.
func (p *TypeParser) GenerateCompleteCSharp(namespace string) (string, error) {
	var buf strings.Builder

	// Check if we need any using statements
	allTypes := p.registry.GetAll()
	needsJsonSerialization := false
	for _, node := range allTypes {
		if node.GetKind() == TypeKindObject {
	...
	for i, name := range typeNames {
		...
		if err != nil {
			return "", fmt.Errorf("failed to generate C# code for %s: %w", name, err)
		}
		...
		// Add blank line between types (except after last one)
